...
file_pref_PROM= 'harvest_PROM_surv'
file_pref_spont= 'harvest_spont_surv'
final_vars_PROM= c('SentrixID_1', 'SVLEN_UL_DG', 'PROM', 'PC1', 'PC2', 'PC3', 'PC4', 'PC5', 'PC6', 'PARITY0','BATCH')
final_vars_spont= c('SentrixID_1', 'SVLEN_UL_DG', 'spont', 'PC1', 'PC2', 'PC3', 'PC4', 'PC5', 'PC6', 'PARITY0','BATCH')
...
final_moms= inner_join(final, pc_moms, by= 'PREG_ID_1724') %>% select(final_vars_PROM)
final_fets= inner_join(final, pc_fets, by= 'PREG_ID_1724') %>% select(final_vars_PROM)

final_moms= final_moms[!duplicated(final_moms$SentrixID_1),]
final_fets= final_fets[!duplicated(final_fets$SentrixID_1),]

final_sens_moms= inner_join(final_sens, pc_moms, by= 'PREG_ID_1724') %>% select(final_vars_PROM)
final_sens_fets= inner_join(final_sens, pc_fets, by= 'PREG_ID_1724') %>% select(final_vars_PROM)
